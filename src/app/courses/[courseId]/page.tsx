...
export default function CourseDetailPage() {
  ...
  const handleChapterUpdated = (updatedChapter: Chapter) => {
    if (course) {
        const newCourse = { ...course };
        const chapterIndex = newCourse.chapters.findIndex(c => c.id === updatedChapter.id);
        if (chapterIndex !== -1) {
            newCourse.chapters[chapterIndex] = { ...newCourse.chapters[chapterIndex], ...updatedChapter };
            setCourse(newCourse);
        }
    }
  }
  ...
  const handleLessonUpdated = (updatedLesson: Lesson) => {
    if (course) {
        ...
        for (const chapter of newCourse.chapters) {
            const lessonIndex = chapter.lessons.findIndex(l => l.id === updatedLesson.id);
            if (lessonIndex !== -1) {
                chapter.lessons[lessonIndex] = { ...chapter.lessons[lessonIndex], ...updatedLesson };
                setCourse(newCourse);
                break;
            }
        }
  ...
  const handleDeleteChapter = async (chapterId: string) => {
    if (course) {
        const newCourse = { ...course, chapters: course.chapters.filter(c => c.id !== chapterId) };
        setCourse(newCourse);
    ...
    <div className="p-6">
      ...
      {editMode && (
        <div className="mb-6 flex gap-2">
          ...
          <AddChapterDialog courseId={course.id} onChapterAdded={handleChapterAdded} />
        </div>
      )}
            ...
            <div className="flex items-center justify-between w-full pr-4">
                ...
                {editMode && (
                  <div className="flex items-center gap-2 ml-4">
                    <EditChapterDialog chapter={chapter} onChapterUpdated={handleChapterUpdated} />
                    <DeleteConfirmationDialog onDelete={() => handleDeleteChapter(chapter.id)} itemName={`главу "${chapter.title}"`} />
                    <AddLessonDialog chapterId={chapter.id} onLessonAdded={handleLessonAdded} />
                  </div>
                )}
            </div>
            <AccordionContent>
              <ul className="space-y-2 pt-2">
                {chapter.lessons.map(lesson => (
                  <li key={lesson.id} className="flex items-center justify-between p-2 rounded-md hover:bg-gray-100 dark:hover:bg-gray-800">
                    <Link href={`/courses/${course.id}/lessons/${lesson.id}`} className="flex items-center flex-1">
                        {lesson.completed ? (
                    ...
                    {editMode && (
                      <div className="flex items-center gap-2">
                        <EditLessonDialog lesson={lesson} onLessonUpdated={handleLessonUpdated} />
                        <DeleteConfirmationDialog onDelete={() => handleDeleteLesson(lesson.id)} itemName={`урок "${lesson.title}"`} />
                      </div>
                    )}
                  </li>
                ))}
