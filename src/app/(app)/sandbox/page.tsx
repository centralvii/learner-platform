...
import { CheckCircle, XCircle, Circle, Edit, Trash2, PlusCircle } from 'lucide-react'
  ...
  return (
    <div className="p-6">
        <header className="mb-8 flex justify-between items-start">
            <div>
                <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Задания SQL тренажёра</h1>
                <p className="mt-2 text-gray-600 dark:text-gray-300">Упражнения по SQL, приближенные к реальным профессиональным задачам</p>
            </div>
        ...
        <div className="flex items-center mb-4 space-x-4">
            <div className="flex items-center space-x-2">
                ...
                <span>Лёгкие: {tasks.filter(t => t.difficulty === 'easy').length}</span>
            </div>
            <div className="flex items-center space-x-2">
                <span className="w-3 h-3 rounded-full bg-yellow-500"></span>
                <span>Средние: {tasks.filter(t => t.difficulty === 'medium').length}</span>
            </div>
            <div className="flex items-center space-x-2">
                <span className="w-3 h-3 rounded-full bg-red-500"></span>
                <span>Сложные: {tasks.filter(t => t.difficulty === 'hard').length}</span>
            </div>
        </div>
        ...
        <div className="flex gap-4 mb-8">
            ...
            <Select value={sortBy} onValueChange={setSortBy}>
                ...
                <SelectContent>
                    ...
                    <SelectItem value="difficulty-asc">Сначала легкие</SelectItem>
                    <SelectItem value="difficulty-desc">Сначала сложные</SelectItem>
                </SelectContent>
            </Select>
            {editMode && <Button onClick={() => setIsCreating(true)}><PlusCircle className="mr-2 h-4 w-4" /> Добавить задачу</Button>}
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-muted-foreground mt-2">По вашему запросу ничего не найдено.</p>
        </div>
      ) : (
        <div className="border rounded-lg">
          {filteredTasks.map((task, index) => (
            <div key={task.id} className="flex items-center justify-between p-4 border-b last:border-b-0">
                <Link href={`/sandbox/${task.id}`} className="flex items-center flex-grow">
                    <span className="text-lg font-medium text-gray-400 mr-6">{index + 1}</span>
                    <div>
                ...
                <div className="flex items-center gap-2">
                    ...
                    {editMode && (
                        <>
                            ...
                            <DeleteConfirmationDialog onDelete={() => handleDelete(task.id)} itemName={`задачу "${task.title}"`} />
                        </>
                    )}
                </div>
            </div>
        ...
        <Dialog open={isCreating || !!editingTask} onOpenChange={() => { setIsCreating(false); setEditingTask(null); }}>
