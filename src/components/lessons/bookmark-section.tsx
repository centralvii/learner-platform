          ...
          <div className="space-y-2">
            {loading ? (
              ...
              bookmarks.map(bookmark => (
                <div key={bookmark.id} className="p-2 bg-gray-100 dark:bg-gray-800 rounded-md text-sm flex items-center">
                  <Bookmark className="h-4 w-4 mr-2" />
                  <span>{bookmark.title}</span>
                </div>
              ))
            ) : (
              <p className="text-sm text-muted-foreground">У вас пока нет закладок к этому уроку.</p>
            )}
          </div>
        </div>
      </CardContent>
